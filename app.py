...
with tab1:
    ...
    if order_id:
        ...
        if route:
            ...
            cost_per_km_per_unit = 10  # ₹10 per km per unit
            wh_speed = 30  # km/h from warehouse
            st_speed = 40  # km/h from store

            total_dist = total_cost = est_time = 0.0
            for i in range(len(route) - 1):
                loc1 = get_location(route[i][1])
                loc2 = get_location(route[i+1][1])
                leg_dist = geodesic(loc1, loc2).km
                speed = wh_speed if route[i][0] == "Warehouse" else st_speed
                cost  = leg_dist * cost_per_km_per_unit * quantity

                total_dist += leg_dist
                total_cost += cost
                est_time   += leg_dist / speed

            st.write(f"Total Distance: {total_dist:.2f} km")
            st.write(f"Estimated Delivery Time: {est_time:.1f} hrs")
            st.write(f"Total Delivery Cost: Rs {total_cost:.2f}")
...
# --------------------------------------------------------------------------
# TAB 2  –  Unified AI‑Driven Retail Forecasting Dashboard
# --------------------------------------------------------------------------
with tab2:
